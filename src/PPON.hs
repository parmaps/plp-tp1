...
pponObjetoSimple pepon = case pepon of 
                            TextoPP _ -> False
                            IntPP _ -> False
                            ObjetoPP xs -> all (\ x-> pponAtomico (snd x)) xs
...
-- La recursión que utiliza es primitiva y NO estructural ya que
-- se le pasa pepon (la subestructura) a pponObjetoSimple, 
-- lo cual hace que acceda a la subestructura sin modificar,
-- y este comportamiento no es válido en la recursión estructural 
-- Por otro lado, también cumple con que se accede a la subestructura en el llamado recursivo
pponADoc :: PPON -> Doc
